...
import Two_Column_Section from "~/components/Sections/Two_Column_Section.vue";
...
onMounted(async () => {
    try {
        const res = await fetch("https://admin.dspcrm.com/wp-json/myapi/v1/page-data/652");
    ...
    console.log("My Data:", smmPageDatas.value);
        sections.value = [
            ...
            {
                comp: SmmClientsBanner,
                 props: { banner: smmPageDatas.value.Team_member2,
                 sectionId: "section-1"
                  }
            },
            {
    comp : CTA,
   props: { sectionId: "CTA-2" }
      },
            {
                comp: Two_Column_Section,
                 props: { banner: smmPageDatas.value.Two_columns}
            },
            {
                comp: Logos, 
